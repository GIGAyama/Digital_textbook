...
export default function App() {
  ...
  const scanQRCode = useCallback((rect) => {
    if (!fabricRef.current || !window.jsQR) return;
    ...
    img.onload = () => {
      ...
      const padding = 20;
      let sx = Math.max(0, rect.left - padding);
      let sy = Math.max(0, rect.top - padding);
      let sw = Math.min(img.width - sx, rect.width + padding * 2);
      let sh = Math.min(img.height - sy, rect.height + padding * 2);
