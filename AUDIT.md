...
### 1. maskable アイコンがセーフゾーンを侵している

`pwa-maskable-512x512.png` は、実体が**通常の角丸アイコン**であり、真の maskable ではない。
中央80%の円の外側にある画素のうち **21.1%** が背景（白）と異なる＝絵が食い込んでいる。
Android がしずく型・円形などで切り抜くと、青い角丸四角の角が欠ける。

正しく直すには、絵柄を中央80%に収め、外周を単色で埋める必要がある。
これは**アイコンの見た目そのものを変える**ため、改修モードの原則（配色・意匠は
報告に留める）に従い実施していない。**別途ご判断ください。**

### 2. `src/App.jsx` の分割

3,247行・175KB で基準（5,000行 / 400KB）内には収まっている。
`src/features/`（`textbook` / `canvas` / `share` / `drive` / `stamps`）への分割が
望ましいが、状態が広く共有されており機械的に切ると壊れる。
**1機能ずつ、合意のうえで分けることを提案する。**

### 3. コントラストの全画面実測

本文級に使われていた薄い文字色は1段濃くしたが、全画面の網羅的な計測は行っていない。
実機の Chromebook で気になる箇所があればご指摘いただきたい。
